...
const Contact = () => {
  ...
  // Modal Animation
  useEffect(() => {
    if (popup.show && modalRef.current) {
      gsap.fromTo(
        modalRef.current,
  ...
  const validate = () => {
    ...
    if (!form.message.trim() || form.message.length < 10) errs.message = "Au moins 10 caractères requis";
    setErrors(errs);
    ...
    try {
      const response = await fetch(N8N_WEBHOOK_URL, {
        ...
        body: JSON.stringify({
          ...
          subject: form.subject || "Contact Portfolio",
      ...
      setPopup({
        ...
        message: "Merci d'avoir pris contact ! Je vous répondrai dans les plus brefs délais.",
      });
      ...
      setPopup({
        ...
        message: "Erreur réseau. Contactez-moi directement par e-mail ou WhatsApp.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const contactCards = [
    ...
    { icon: <FaWhatsapp className="text-emerald-600" size={16} />, label: "WhatsApp", val: "+261 32 61 675 99", link: "https://wa.me/261326167599" },
  ...
  return (
    <div ref={sectionRef} className="relative w-full max-w-5xl mx-auto px-4 sm:px-6 py-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start w-full">
        {/* Coordonnées */}
        <div ref={infoCardsRef} className="lg:col-span-5 space-y-4">
          <div className="glass-panel p-5 sm:p-7 rounded-2xl space-y-5 shadow-sm">
            ...
            <div className="space-y-3">
              {contactCards.map((card, idx) => (
                <div
                  key={idx}
                  className="flex items-center gap-3 p-3.5 rounded-xl bg-white border border-slate-200 hover:border-emerald-300 hover:shadow-md hover:scale-[1.01] transition-all duration-200"
                >
                  <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200">{card.icon}</div>
                  <div className="flex-1 min-w-0">
                    ...
                    {card.link ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-xs sm:text-sm font-semibold text-slate-900 hover:text-emerald-600 transition-colors truncate block"
                      >
                        {card.val}
                      </a>
                    ) : (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-slate-700 mb-1.5">
                    Nom &amp; Prénom <span className="text-emerald-600">*</span>
                  </label>
                  <input
                    type="text"
                    name="nom"
                    ...
                    className={`w-full bg-white border rounded-xl px-4 py-2.5 text-xs sm:text-sm text-slate-900 placeholder-slate-400 outline-none transition-all shadow-sm ${
                      errors.nom
              ...
              <div>
                ...
                <input
                  ...
                  value={form.subject}
                  onChange={handleChange}
                  placeholder="Création d'une application / Mission freelance"
              ...
              <div>
                ...
                <textarea
                  ...
                  className={`w-full bg-white border rounded-xl px-4 py-2.5 text-xs sm:text-sm text-slate-900 placeholder-slate-400 outline-none transition-all resize-none shadow-sm ${
                    errors.message
                      ? "border-red-400 focus:ring-2 focus:ring-red-400/20"
                      : "border-slate-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20"
                  }`}
      ...
      {popup.show && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div
            ref={modalRef}
            className="relative w-full max-w-md bg-white border border-slate-200 rounded-2xl p-6 shadow-2xl text-center flex flex-col items-center"
          >
            <button
            ...
            <button
              onClick={() => setPopup({ ...popup, show: false })}
              className="w-full py-2.5 px-4 rounded-xl bg-slate-100 hover:bg-slate-200 text-xs font-semibold text-slate-900 transition-colors"
            >
              Fermer
            </button>
          </div>
